...
class Aplication(Funcoes_Player):
    ...
    def update_music_infor(self):
        ...
        print(f"duration_secunds - {duration_minutes}:{str_seconds} \n",  end='\r')

        print(self.duration_music, end='\r')
        self.label_end.configure(text=f'{duration_minutes}:{str_seconds}')

    def update_bar_progress(self):
        self.bar_progress_value = 1
        self.pos_music += 1
        print('seg ',self.pos_music)
        counter = math.trunc(float(self.get_pos_music()))
        minutes = counter // 60
        seconds = counter % 60
        self.bar_progress_value += counter

        print(f"bar progress - {minutes}:{seconds} \n",  end='\r')
        if self.duration_music < 1:
            self.duration_music = 1
        print('duration_music --', self.duration_music)
        print('counter --', counter)
        print(f'progress {math.trunc((counter * 100) / self.duration_music)}')
    ...
    def slide_set_volume(self, event):
        print('set_volume', format(self.bar_volume.get() / 100, '.2f'))
    ...
    def slider_pos_music(self, event):
        ...
        print(value_progress_bar)
        self.set_pos_music(value_progress_bar)
        print(f'progress {math.trunc((value_progress_bar * 100) / self.duration_music)}')
    ...
    def widgets_frame2(self):
        ...
        self.label_init = Label(self.frame2, text=str(self.get_pos_music()), bg="#583bbf", fg="white", font=('verdana', 9, 'bold'))
